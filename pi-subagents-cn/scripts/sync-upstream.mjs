 ...
 *
 * 流程：
 *   1. 查询 npm 上 @narumitw/pi-subagents 最新版本
...
let upstreamVersion;
try {
  upstreamVersion = execSync("npm view @narumitw/pi-subagents version", {
    encoding: "utf-8",
    timeout: 60000,
  }).trim();
} catch {
...
try {
  execSync(`npm pack @narumitw/pi-subagents@${upstreamVersion} --pack-destination ${WORK_DIR}`, {
    stdio: "pipe",
    timeout: 120000,
  });
} catch {

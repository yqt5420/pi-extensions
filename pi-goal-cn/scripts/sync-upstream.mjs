 ...
 *
 * 流程：
 *   1. 查询 npm 上 @narumitw/pi-goal 最新版本
...
let upstreamVersion;
try {
  upstreamVersion = execSync("npm view @narumitw/pi-goal version", {
    encoding: "utf-8",
    timeout: 60000,
  }).trim();
} catch {
...
try {
  execSync(`npm pack @narumitw/pi-goal@${upstreamVersion} --pack-destination ${WORK_DIR}`, {
    stdio: "pipe",
    timeout: 120000,
  });
} catch {
